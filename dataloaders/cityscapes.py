...
class Cityscapes(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, root, split="train", transform=None, transform_target=False):
        ...
        print(f"Cityscapes {split} with {len(self.images)} images")

    @classmethod
    def encode_target(cls, target):
        return cls.id_to_train_id[np.asarray(target)]
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (int): Index
        Returns:
            tuple: (image, target, meta).
        """
        path = self.images[index]
        if "train/" in path:
            path = path.replace("train/", "train_jpg/").replace(".png", ".jpg")
