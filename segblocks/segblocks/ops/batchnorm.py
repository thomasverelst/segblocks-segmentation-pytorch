...
def block_batch_norm(dualres: 'DualResTensor', running_mean: torch.Tensor, running_var: torch.Tensor, 
                     weight: torch.Tensor, bias: torch.Tensor, training: bool, momentum: float, eps: float):
    ...
    if dualres.metadata.nhighres > 0:
        hr = torch.nn.functional.batch_norm(hr, running_mean, running_var, weight, bias, training=training, momentum=0, eps=eps)

    if dualres.metadata.nlowres > 0:
        lr = torch.nn.functional.batch_norm(lr, running_mean, running_var, weight, bias, training=training, momentum=0, eps=eps)
