# efficientnet-lite from https://github.com/RangiLyu/EfficientNet-Lite
...
def drop_connect(x, drop_connect_rate, training):
    ...
    random_tensor += torch.rand(tuple([batch_size, 1, 1, 1]), dtype=x.dtype, device=x.device)
...
class EfficientNetLite(nn.Module):
    def __init__(self, widthi_multiplier, depth_multiplier, num_classes, drop_connect_rate, dropout_rate, skip_last_stride=False, do_max_pool=True, strides=None):
        super(EfficientNetLite, self).__init__()
        if strides is None:
            strides = [2,2,2,2]
        else:
            skip_last_stride = strides[-1] == 1
        assert do_max_pool is True
        ...
        mb_block_settings = [
            #repeat|kernal_size|stride|expand|input|output|se_ratio
                ...
                [3, 3, 2, 6, 40,  80,  0.25],
                [3, 5, 1, 6, 80,  112, 0.25],
                [4, 5, 2 if not skip_last_stride else 1, 6, 112, 192, 0.25],
        ...
        self.block_features = []
        self.block_feature_stage_keep_features = []
        ...
        self.block_features.append(output_filters)
        self.block_feature_stage_keep_features.append(True)
        self.block_features = self.block_features[-4 if not skip_last_stride else -3:]
        self.block_feature_stage_keep_features = self.block_feature_stage_keep_features[1:]
        self.block_feature_stage_keep_features[0] = False
    ...
    def forward_features(self, x):
        ...
        for i, stage in enumerate(self.blocks):
            for block in stage:
                ...
                x = block(x, drop_connect_rate)
                idx +=1
            if self.block_feature_stage_keep_features[i]:
                features.append(x)
        return features
    ...
    def load_pretrain(self, path):
        ...
        print(f'Loaded backbone pretraining from {path}')
...
model_urls = {
    ...
    'efficientnet_lite0': 'pretrained/backbones/efficientnet_lite0.pth',
    'efficientnet_lite1': 'pretrained/backbones/efficientnet_lite1.pth',
    'efficientnet_lite2': 'pretrained/backbones/efficientnet_lite2.pth',
    'efficientnet_lite3': 'pretrained/backbones/efficientnet_lite3.pth'
}


def efficientnet_lite0(pretrained=False, **kwargs):
    """Constructs a EfficientNet-Lite-1 model.
    """
    model = build_efficientnet_lite('efficientnet_lite0', **kwargs)
    if pretrained:
        model.load_pretrain(model_urls['efficientnet_lite0'])
    return model

def efficientnet_lite1(pretrained=False, **kwargs):
    """Constructs a EfficientNet-Lite-1 model.
    """
    model = build_efficientnet_lite('efficientnet_lite1', **kwargs)
    if pretrained:
        model.load_pretrain(model_urls['efficientnet_lite1'])
    return model

def efficientnet_lite2(pretrained=False, **kwargs):
    """Constructs a EfficientNet-Lite-1 model.
    """
    model = build_efficientnet_lite('efficientnet_lite2', **kwargs)
    if pretrained:
        model.load_pretrain(model_urls['efficientnet_lite2'])
    return model

def efficientnet_lite3(pretrained=False, **kwargs):
    """Constructs a EfficientNet-Lite-1 model.
    """
    model = build_efficientnet_lite('efficientnet_lite3', **kwargs)
    if pretrained:
