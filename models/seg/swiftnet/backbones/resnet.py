...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = nn.Sequential(self.conv2, self.bn2)(out)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, do_max_pool=True, strides=None, dilates=None):
        ...
        # self.block_features = [64,128,256,512]
    ...
    def forward_features(self, x):
        x = nn.Sequential(self.conv1, self.bn1, self.relu)(x)
        if self.do_max_pool:
            x = self.maxpool(x)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    logger.info(f'Model: ResNet {arch}')
    if pretrained:
        logger.info(f'Model: Loading pretrained Imagenet model')
